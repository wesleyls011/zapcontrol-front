...
import AddEstoque, {
  ...
  type EstoqueItem as AddEstoqueItem,
} from "./AddEstoque";
...
const Estoque: React.FC = () => {
  ...
  const handleSaveItem = (data: EstoqueFormData, id?: number) => {
    if (id !== undefined && editingItem && editingItem.id === id) {
      setItems(
        items.map((item) => (item.id === id ? { ...item, ...data, id } : item))
      );
    } else {
  ...
  const getStatusColor = (item: AddEstoqueItem) => {
    ...
    if (item.quantidade <= item.minimo * 2)
              ...
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-2">
                  ...
                  <button
                    ...
                    className="text-red-600 hover:text-red-800 flex items-center gap-1 pr-1"
                  >
                    Remover
